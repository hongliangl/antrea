...
function setup_external_frr {
  if [[ $DEPLOY_EXTERNAL_FRR == true ]]; then
    docker run -d --name antrea-external-frr-$RANDOM --network kind -it --rm frrouting/frr:v8.4.0 bash -c "/bin/sed -i s/bgpd=no/bgpd=yes/g /etc/frr/daemons && /sbin/tini -- /usr/lib/frr/docker-start" &> /dev/null
  fi
}
