...
function print_aliases {
    printf "All aliases for OF tables are listed below. You can customize your own aliases for OF tables.\n"
    for ALIAS in "${TABLE_ALIAS_LIST[@]}"; do
        printf "       %-10s         %s\n" "$ALIAS" "${TABLE_ALIAS_MAP[$ALIAS]}"
    done

    printf "\nAll aliases for Antrea Agent Pod are listed below, You can customize your own aliases for some frequently used Antrea Agent Pods.\n"
...
if [[ "windows" =~ $AGENT_POD ]]; then
    NODE_IP=$(kubectl get pods -o wide -n kube-system --no-headers | grep $AGENT_POD | awk '{print $6}')
    $WATCH_ARG ssh administrator@$NODE_IP "ovs-ofctl.exe dump-flows br-int $TABLE_ARG $STAT_ARG $NAMES_ARG $PROTO_ARG"
else
