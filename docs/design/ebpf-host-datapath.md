...
The datapath has two modes, set by `ebpfHostDataPath.mode` in the Agent configuration. In `observe` mode,
the default, the programs make every decision they make in `forward` mode and count it as
`would_forward` or `would_return`, but leave every packet to the host network stack untouched. In `forward`
mode they forward the packets. The counters and the mode are exported to Prometheus as
`antrea_agent_ebpf_host_datapath_packets_total` and `antrea_agent_ebpf_host_datapath_mode`.
...
1. **Reattachment is periodic.** The Agent looks the gateway and the transport interface up by name every 30
   seconds. When one was recreated with another index, or its traffic control filter was removed, the program
   is attached again and the reattachment is logged. Until that check, the programs do not run on the new
   interface and its traffic falls back to the host network stack. An interface recreated with the same index
   between two checks is not noticed when a BPF link is used, as the index is all the check compares.
2. **The MTUs are rechecked with the interfaces.** A changed MTU is written into `node_config` at the next
   check. Until then the size check uses the old value, although `bpf_fib_lookup` performs its own check and
   catches the case, which was observed in testing.
...
6. **Enabling the gate on only one side of the cluster is fine, but enabling it in the wrong mode is only
