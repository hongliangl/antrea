...
func newFakeExec(err error) *exectesting.FakeExec {
	fcmd := exectesting.FakeCmd{
		RunScript: []exectesting.FakeAction{
...
func TestRun(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "sysctl command failure",
			fs:   afero.NewMemMapFs(),
			exec: newFakeExec(fmt.Errorf("some error")),
			err:  "failed to apply antrea sysctl configuration",
		},
	}
