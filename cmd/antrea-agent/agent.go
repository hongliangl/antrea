...
func loadEBPFHostDataPath(mode hostdp.Mode, networkConfig *config.NetworkConfig, nodeConfig *config.NodeConfig) (hostdp.Interface, error) {
...
func run(o *Options) error {
	...
	if features.DefaultFeatureGate.Enabled(features.EBPFHostDataPath) && o.nodeType == config.K8sNode {
		mode, err := hostdp.ParseMode(o.config.EBPFHostDataPath.Mode)
		if err != nil {
			return err
		}
		client, err := loadEBPFHostDataPath(mode, networkConfig, nodeConfig)
		if err != nil {
			...
			go hostDPClient.Run(stopCh)
			if *o.config.EnablePrometheusMetrics {
				metrics.InitializeEBPFHostDataPathMetrics(hostDPClient, mode)
			}
		}
	}
