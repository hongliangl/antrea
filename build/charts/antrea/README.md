...
| ebpfHostDataPath.mode | string | `"observe"` | Mode of the eBPF host datapath, which the EBPFHostDataPath feature gate enables. It is either "observe" or "forward". In "observe" mode the eBPF programs count what they would forward and forward nothing. In "forward" mode they forward the traffic between the Pods of two Nodes. |
