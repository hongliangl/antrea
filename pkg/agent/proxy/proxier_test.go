...
func testLoadBalancer(t *testing.T, nodePortIPMap map[int][]net.IP, svcIP, ep1IP, ep2IP, loadBalancerIP net.IP, isIPv6, nodeLocalExternal bool) {
	...
	mockOFClient.EXPECT().InstallServiceClassifierFlow(groupID, loadBalancerIP, uint16(svcPort), bindingProtocol, uint16(0), nodeLocalExternal).Times(1)
	mockOFClient.EXPECT().InstallServiceClassifierFlow(groupID, svcNodePortIP, uint16(svcNodePort), bindingProtocol, uint16(0), nodeLocalExternal).Times(1)
	mockRouteClient.EXPECT().AddClusterIPRoute(svcIP, isIPv6).Times(1)
