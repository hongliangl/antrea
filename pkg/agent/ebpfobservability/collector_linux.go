...
func (c *Collector) Run(ctx context.Context) error {
	...
	if err := loadConnectObjects(&objects); err != nil {
		return err
	}
	defer objects.Close()
...
func (c *Collector) logEvent(event Event) {
	...
	agentmetrics.EBPFEventsTotal.WithLabelValues(event.Type.String(), attributed).Inc()
	if event.Type == EventTypeTCPRTT && event.SRTTUS != 0 {
		agentmetrics.EBPFTCPSRTT.Observe(float64(event.SRTTUS) / 1_000_000)
	}
	if !found {
