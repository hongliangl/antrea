...
#define CFG_MODE             6 // whether the programs send packets or only count them, see MODE_ below

// The values of the CFG_MODE slot. In observe mode the programs make every decision they make in forward mode
// and count it, but leave every packet to the stack untouched. Observe is 0, so that a slot which was never
// written, or which cannot be read, means observe rather than forward.
...
#define STAT_WOULD_FWD    7 // observe mode: would have been redirected to the peer Node, left to the stack
#define STAT_WOULD_RETURN 8 // observe mode: would have been redirected to the gateway, left to the stack
...
struct hostdp_map_def node_config __attribute__((section(".hostdp_maps"), used)) = {
	...
	.max_entries = 7, // one per CFG_ slot above
	.map_flags = 0,
};
...
struct hostdp_map_def stats __attribute__((section(".hostdp_maps"), used)) = {
	...
	.max_entries = 9, // one per STAT_ slot above
	.map_flags = 0,
};
...
//
// In observe mode it stops once the lookup succeeded, counts would_stat, and leaves the packet to the stack
// without having changed a byte of it.
//
// A return of TC_ACT_OK leaves the packet to the stack. Anything else means it has been sent.
//
// A function the verifier sees as a function takes five arguments at most, as there are five registers to
// pass them in. This one takes six, which is fine only because always_inline expands it into its callers.
static inline __attribute__((always_inline)) int redirect_via(struct __sk_buff *skb, struct iphdr *ip,
                                                              ...
                                                              __u32 would_stat)
{
	// skb->len counts the Ethernet header as well, as the packet still carries it at this hook, while the MTU
	// is the largest IP packet the interface takes, without that header. Comparing them as they are would
	// leave every packet of full size to the stack, and the packets of a bulk TCP transfer are all of full
	// size. parse_ipv4 has already checked that both headers are there, so the subtraction cannot wrap.
	if (mtu != 0 && skb->len - ETH_HLEN > mtu) {
	...
	// This is the point where forwarding starts changing the packet, so observe mode stops right before it.
	// A packet which got this far is one forward mode would have sent, and one which did not get this far
	// was counted above exactly as in forward mode, so the miss counters mean the same in both modes. Any
	// value other than MODE_FORWARD is taken as observe, so that a mistake leaves packets alone.
	if (cfg(CFG_MODE) != MODE_FORWARD) {
		count(would_stat);
		return TC_ACT_OK;
	}
	if (dec_ttl(skb, ip) < 0) {
...
__attribute__((section("tc"), used)) int hostdp_fwd(struct __sk_buff *skb)
{
	...
	int ret = redirect_via(skb, ip, next_hop, cfg(CFG_TRANSPORT_MTU), STAT_FWD_MISS, STAT_WOULD_FWD);
	if (ret != TC_ACT_OK)
		count(STAT_FWD);
	return ret;
}
...
__attribute__((section("tc"), used)) int hostdp_return(struct __sk_buff *skb)
{
	...
	int ret = redirect_via(skb, ip, ip->daddr, cfg(CFG_GATEWAY_MTU), STAT_RETURN_MISS, STAT_WOULD_RETURN);
	if (ret != TC_ACT_OK)
		count(STAT_RETURN);
	return ret;
}
