...
func ipBlocksToIPNets(ipBlocks []v1beta2.IPBlock, isIPv6 bool) sets.Set[string] {
	...
	for _, b := range ipBlocks {
		...
		for _, d := range diffCIDRs {
			if ip.IsZeroCIDR(d) {
				// Due to the inability to add 0.0.0.0/0 and ::/0 to an ipset, divide them into two separate CIDRs.
				if utilnet.IsIPv6CIDR(blockCIDR) {
					ipNets.Insert("::/1", "8000::/1")
				} else {
					ipNets.Insert("0.0.0.0/1", "128.0.0.0/1")
				}
			} else {
