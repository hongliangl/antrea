...
func (c *Controller) syncRule(key string) error {
	...
	if len(rule.L7Protocols) != 0 {
		...
		rule.L7RuleVlanID = c.l7VlanIDAllocator.allocate(key)

		if err := c.l7RuleReconciler.AddRule(key, rule.SourceRef.ToString(), rule.L7RuleVlanID, rule.L7Protocols); err != nil {
			return err
		}
	}
