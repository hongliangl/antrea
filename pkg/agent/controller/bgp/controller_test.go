...
func TestBGPPolicyAdd(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			ipv4Enabled: true,
			bpsToAdd: []runtime.Object{generateBGPPolicy(bgpPolicyName1,
				creationTimestamp,
				nodeLabels1,
				179,
				65000,
		...
		},
		{
			...
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				...
				mockBGPServer.AddPeer(ctx, ipv4Peer1Config)
				mockBGPServer.AddPeer(ctx, ipv6Peer1Config)
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv4)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv6)}})
			},
		},
		{
		...
		},
		{
			...
			ipv4Enabled: true,
			bpsToAdd: []runtime.Object{generateBGPPolicy(bgpPolicyName2,
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			c := newFakeController(t, tt.objects, append(tt.crdObjects, tt.bpsToAdd...), tt.ipv4Enabled, tt.ipv6Enabled)
			...
			waitAndDoneDummyEvent(t, c)

			if tt.expectedCalls != nil {
				tt.expectedCalls(c.mockBGPServer.EXPECT())
			}
			if tt.expectedError != "" {
				assert.EqualError(t, c.syncBGPPolicy(), tt.expectedError)
			} else {
...
func TestBGPPolicyUpdate(t *testing.T) {
	...
	effectiveBPState := generateBGPPolicyState(179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{ipStrToPrefix(clusterIPv4),
	...
	alternativeBP := generateBGPPolicy(bgpPolicyName2,
		creationTimestampAdd1s,
		nodeLabels1,
		1179,
		65000,
	...
	unrelatedBP := generateBGPPolicy(bgpPolicyName3,
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				...
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(clusterIPv4)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(clusterIPv6)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv4)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv6)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: podIPv4CIDR.String()}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: podIPv6CIDR.String()}})
			},
			expectedState: generateBGPPolicyState(1179,
				65000,
		...
		{
			name: "Effective BGPPolicy, update NodeSelector (not applied to current Node), failed to stop stale BGP server",
			bpToUpdate: generateBGPPolicy(bgpPolicyName1,
				creationTimestamp,
				nodeLabels2,
				179,
			...
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.Stop(ctx).Return(fmt.Errorf("failed to stop"))
			},
			expectedState: deepCopyBGPPolicyState(effectiveBPState),
			expectedError: "failed to stop stale BGP server: failed to stop",
		},
		{
			...
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(externalIPv4)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(ipv4EgressIP1)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(externalIPv6)}})
				mockBGPServer.AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(ipv6EgressIP1)}})

				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(clusterIPv4)}})
				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv4)}})
				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: podIPv4CIDR.String()}})
				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(clusterIPv6)}})
				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv6)}})
				mockBGPServer.WithdrawRoutes(ctx, []bgp.Route{{Prefix: podIPv6CIDR.String()}})
			},
		},
		{
		...
		},
		{
			...
			existingState: deepCopyBGPPolicyState(effectiveBPState),
			expectedState: deepCopyBGPPolicyState(effectiveBPState),
		},
		{
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.bgpPolicyState = deepCopyBGPPolicyState(effectiveBPState)
			c.bgpPolicyState.bgpServer = c.mockBGPServer
			c.bgpPeerPasswords = bgpPeerPasswords
			...
			// Wait for the dummy event triggered by BGPPolicy update events, and mark it done.
			waitAndDoneDummyEvent(t, c)
...
func TestBGPPolicyDelete(t *testing.T) {
	...
	bp1State := generateBGPPolicyState(179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{
			ipStrToPrefix(loadBalancerIPv4),
	...
	bp2State := generateBGPPolicyState(179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{
	...
	bp3State := generateBGPPolicyState(1179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{
			ipStrToPrefix(externalIPv4),
	...
	testCases := []struct {
	...
	}{
		{
			...
			existingState: deepCopyBGPPolicyState(bp1State),
			expectedState: nil,
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.Stop(ctx)
			},
		},
		{
			...
			existingState: deepCopyBGPPolicyState(bp1State),
			expectedState: deepCopyBGPPolicyState(bp2State),
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.RemovePeer(ctx, ipv4Peer1Config)
				mockBGPServer.RemovePeer(ctx, ipv6Peer1Config)
				mockBGPServer.AddPeer(ctx, ipv4Peer2Config)
		...
		},
		{
			...
			bpToDelete:    bgpPolicyName1,
			crdObjects:    []runtime.Object{bp1, bp3},
			existingState: deepCopyBGPPolicyState(bp1State),
			expectedState: deepCopyBGPPolicyState(bp3State),
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.Stop(ctx)
				mockBGPServer.Start(ctx)
				mockBGPServer.AddPeer(ctx, ipv4Peer2Config)
		...
		},
		{
			...
			crdObjects:    []runtime.Object{bp1, bp2},
			existingState: deepCopyBGPPolicyState(bp1State),
			expectedState: deepCopyBGPPolicyState(bp1State),
		},
	}
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Wait for the dummy event triggered by BGPPolicy delete events, and mark it done.
			waitAndDoneDummyEvent(t, c)
...
func TestNodeUpdate(t *testing.T) {
	...
	bp2 := generateBGPPolicy(bgpPolicyName2,
		...
		true,
		[]v1alpha1.BGPPeer{ipv4Peer1, ipv6Peer1})
	bp2State := generateBGPPolicyState(1179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{podIPv4CIDR.String(), podIPv6CIDR.String()},
		[]bgp.PeerConfig{ipv4Peer1Config, ipv6Peer1Config})
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Wait for the dummy event triggered by Node update events, and mark it done.
			waitAndDoneDummyEvent(t, c)
...
func TestServiceLifecycle(t *testing.T) {
	...
	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: "10.96.10.10/32"}})
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.100/32"}})
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.150/32"}})
	require.NoError(t, c.syncBGPPolicy())
	...
	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.151/32"}})
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.101/32"}})
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.100/32"}})
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.150/32"}})
	require.NoError(t, c.syncBGPPolicy())
	...
	// withdrawn.
	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.101/32"}})
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: "192.168.77.151/32"}})
	require.NoError(t, c.syncBGPPolicy())
	...
	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: "10.96.10.10/32"}})
	require.NoError(t, c.syncBGPPolicy())
...
func TestSyncBGPPolicyFailures(t *testing.T) {
	...
	bp3 := generateBGPPolicy(bgpPolicyName3,
		...
		false,
		[]v1alpha1.BGPPeer{ipv4Peer2})
	bp4 := generateBGPPolicy(bgpPolicyName4,
	...
	mockBGPServer.EXPECT().Start(ctx)
	mockBGPServer.EXPECT().AddPeer(ctx, ipv4Peer2Config)
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv4)}})
	require.NoError(t, c.syncBGPPolicy())
	...
	// Delete the effective BGPPolicy bp1, and BGPPolicy bp2 will be the effective one.
	require.NoError(t, c.crdClient.CrdV1alpha1().BGPPolicies().Delete(context.TODO(), bp1.Name, metav1.DeleteOptions{}))
	waitAndDoneDummyEvent(t, c)

	// The local ASN of BGPPolicy bp2 is not the same as BGPPolicy bp1, and the stale BGP server should be stopped.
	// Mock that failing in stopping the stale BGP server.
	mockBGPServer.EXPECT().Stop(ctx).Return(fmt.Errorf("failed reason"))
	require.EqualError(t, c.syncBGPPolicy(), "failed to stop stale BGP server: failed reason")
	checkBGPPolicyState(t, generateBGPPolicyState(179,
		65000,
		nodeIPv4Addr.IP.String(),
		[]string{ipStrToPrefix(loadBalancerIPv4)},
	...
	// Mock the retry. Stop the stale BGP server successfully, but fail in starting a new BGP server.
	mockBGPServer.EXPECT().Stop(ctx)
	mockBGPServer.EXPECT().Start(ctx).Return(fmt.Errorf("failed reason"))
	require.EqualError(t, c.syncBGPPolicy(), "failed to start BGP server: failed reason")
	checkBGPPolicyState(t, nil, c.bgpPolicyState)

	// Mock the retry. Start BGP server successfully, but fail in adding BGP peer.
	mockBGPServer.EXPECT().Start(ctx)
	mockBGPServer.EXPECT().AddPeer(ctx, ipv4Peer1Config).Return(fmt.Errorf("failed to add BGP peer"))
	require.EqualError(t, c.syncBGPPolicy(), "failed to add BGP peer")
	...
	mockBGPServer.EXPECT().AddPeer(ctx, ipv4Peer1Config)
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: podIPv4CIDR.String()}}).Return(fmt.Errorf("failed to advertise routes"))
	require.EqualError(t, c.syncBGPPolicy(), "failed to advertise routes")
	...
	// Delete the effective BGPPolicy bp2, and BGPPolicy bp3 will be the effective one. The BGP server doesn't need to
	// be updated. The peers and routes will be reconciled according to the existing BGPPolicy state.
	require.NoError(t, c.crdClient.CrdV1alpha1().BGPPolicies().Delete(context.TODO(), bp2.Name, metav1.DeleteOptions{}))

	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().AddPeer(ctx, ipv4Peer2Config)
	mockBGPServer.EXPECT().RemovePeer(ctx, ipv4Peer1Config)
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(externalIPv4)}})
	...
	// Delete the effective BGPPolicy bp3, and BGPPolicy bp4 will be the effective one. The BGP server doesn't need to
	// be updated. The peers and routes will be reconciled according to the existing BGPPolicy state.
	require.NoError(t, c.crdClient.CrdV1alpha1().BGPPolicies().Delete(context.TODO(), bp3.Name, metav1.DeleteOptions{}))

	waitAndDoneDummyEvent(t, c)
	mockBGPServer.EXPECT().UpdatePeer(ctx, updatedIPv4Peer2Config)
	mockBGPServer.EXPECT().WithdrawRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(externalIPv4)}})
	mockBGPServer.EXPECT().AdvertiseRoutes(ctx, []bgp.Route{{Prefix: ipStrToPrefix(loadBalancerIPv4)}})
...
func waitAndDoneDummyEvent(t *testing.T, c *fakeController) {
	require.Eventually(t, func() bool {
