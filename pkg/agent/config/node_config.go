...
// L7NetworkPolicyConfig includes target and return ofPorts for L7 Antrea NetworkPolicy.
type L7NetworkPolicyConfig struct {
	TargetOFPort uint32 // Matched L7 Antrea NetworkPolicy traffic is forwarded to an application-aware engine via this ofPort.
	ReturnOFPort uint32 // Scanned L7 Antrea NetworkPolicy traffic is returned from an application-aware engine via this ofPort.
}
