...
const (
	// syncDebounceDuration is how long a notification waits, so that several updates in a row share one sync.
	...
	// Deleting a chain is a call to iptables, so a run deletes this many at most. Only disabling
	// NodeNetworkPolicy leaves many at once, and they are empty, so clearing them slowly is fine.
	maxChainsDeletedPerRun = 20
)
...
// triggerIPTablesSync notifies the sync loop that the iptables caches have been updated. The notification
// is delayed, and adding it again while it waits does nothing, so several updates in a row share one sync.
...
// the feature is disabled, which is when every chain is an orphan nothing else would remove.
func (c *Client) cleanupOrphanNodeNetworkPolicyChains() {
	...
	for ipProtocol, chains := range chainsInDatapath {
		...
		for _, chain := range chains {
			...
			if !c.staleNodeNetworkPolicyChains[ipProtocol].Has(chain) {
				// Not an orphan in the previous run, give it a period to be cached.
				continue
			}
			if deleted == maxChainsDeletedPerRun {
				// It stays a candidate, so the next run deletes it without waiting again.
				remaining++
				continue
			}
			// A chain still referenced by a jump rule cannot be deleted; the next run retries.
	...
	if remaining > 0 {
		klog.InfoS("Deleted as many orphan NodeNetworkPolicy chains as one run may, the rest follow",
