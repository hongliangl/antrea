...
func (c *Client) syncIPTables() error {
	...
	var proxyAllIPTablesNatRulesIPv4, proxyAllIPTablesNatRulesIPv6 map[string][]string
	var proxyAllIPTablesRawRulesIPv4, proxyAllIPTablesRawRulesIPv6 map[string][]string
	if c.proxyAll {
		if c.networkConfig.IPv4Enabled {
			...
			proxyAllIPTablesNatRulesIPv4, proxyAllIPTablesRawRulesIPv4 = c.generateProxyAllRules(
				antreaNodePortIPSet,
				antreaExternalIPIPSet,
				serviceIPv4CIDR,
		...
		if c.networkConfig.IPv6Enabled {
			...
			proxyAllIPTablesNatRulesIPv6, proxyAllIPTablesRawRulesIPv6 = c.generateProxyAllRules(
				antreaNodePortIP6Set,
				antreaExternalIPIP6Set,
				serviceIPv6CIDR,
	...
	if c.networkConfig.IPv4Enabled {
		iptablesData := c.restoreIptablesData(c.nodeConfig.PodIPv4CIDR,
			...
			clusterNodeIPSet,
			snatMarkToIPv4,
			proxyAllIPTablesNatRulesIPv4,
			proxyAllIPTablesRawRulesIPv4,
	...
	if c.networkConfig.IPv6Enabled {
		iptablesData := c.restoreIptablesData(c.nodeConfig.PodIPv6CIDR,
			...
			clusterNodeIP6Set,
			snatMarkToIPv6,
			proxyAllIPTablesNatRulesIPv6,
			proxyAllIPTablesRawRulesIPv6,
...
func (c *Client) generateProxyAllRules(nodePortIPSet string,
	...
	isIPv6 bool) (map[string][]string, map[string][]string) {
	var natPreRoutingRules, natOutputRules []string
	if isIPv6 == utilnet.IsIPv6String(c.kubeServiceHost) {
	...
	natRules := map[string][]string{
		antreaPreRoutingChain:  natPreRoutingRules,
		antreaOutputChain:      natOutputRules,
		antreaPostRoutingChain: natPostRoutingRules,
	}
	rawRules := map[string][]string{
		antreaPreRoutingChain: rawPreroutingRules,
	}

	return natRules, rawRules
}
...
func (c *Client) restoreIptablesData(podCIDR *net.IPNet,
	...
	proxyAllNatIPTables map[string][]string,
	proxyAllRawIPTables map[string][]string,
	...
	if c.proxyAll {
		for _, ruleStr := range proxyAllRawIPTables[antreaPreRoutingChain] {
	...
	if c.proxyAll {
		for _, ruleStr := range proxyAllNatIPTables[antreaPreRoutingChain] {
			writeLine(iptablesData, ruleStr)
		}
		writeLine(iptablesData, iptables.MakeChainLine(antreaOutputChain))
		for _, ruleStr := range proxyAllNatIPTables[antreaOutputChain] {
			writeLine(iptablesData, ruleStr)
		}
	}
	...
	if c.proxyAll {
		for _, ruleStr := range proxyAllNatIPTables[antreaPostRoutingChain] {
			writeLine(iptablesData, ruleStr)
		}
	}
