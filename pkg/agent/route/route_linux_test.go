...
func TestCleanupOrphanNodeNetworkPolicyChainsLimit(t *testing.T) {
	const orphanCount = maxChainsDeletedPerRun + 5
	inDatapath := sets.New[string]()
	for i := 0; i < orphanCount; i++ {
		inDatapath.Insert(fmt.Sprintf("ANTREA-POL-RULE%d", i))
	}
	...
	mockIPTables.EXPECT().ListChains(iptables.ProtocolIPv4, iptables.FilterTable).
		DoAndReturn(func(iptables.Protocol, string) (map[iptables.Protocol][]string, error) {
			return map[iptables.Protocol][]string{iptables.ProtocolIPv4: sets.List(inDatapath)}, nil
		}).Times(3)
	mockIPTables.EXPECT().DeleteChain(iptables.ProtocolIPv4, iptables.FilterTable, gomock.Any()).
		DoAndReturn(func(_ iptables.Protocol, _, chain string) error {
			inDatapath.Delete(chain)
			deleted.Insert(chain)
			return nil
		}).AnyTimes()

	c.cleanupOrphanNodeNetworkPolicyChains()
	assert.Empty(t, deleted, "The first run must only record the candidates")

	c.cleanupOrphanNodeNetworkPolicyChains()
	assert.Len(t, deleted, maxChainsDeletedPerRun, "One run deleted more chains than it is allowed to")

	// What was left over is still a candidate, so it is deleted without waiting a further period.
	c.cleanupOrphanNodeNetworkPolicyChains()
	assert.Len(t, deleted, orphanCount, "The chains left over were not deleted by the next run")
}
...
// A notification is delayed, so it is not in the queue immediately.
