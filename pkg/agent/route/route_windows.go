...
// Run periodically syncs netNatStaticMapping and route. It will not return until ctx is closed.
func (c *Client) Run(ctx context.Context) {
	klog.InfoS("Starting netNatStaticMapping and route sync", "interval", SyncInterval)
	wait.UntilWithContext(ctx, c.syncNetworkInfra, SyncInterval)
}

// syncNetworkConfig is idempotent and can be safely called on every sync operation.
func (c *Client) syncNetworkInfra(ctx context.Context) {
