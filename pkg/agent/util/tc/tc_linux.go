...
func (c *Client) QdiscCheck(handle uint32, ifIndex int) (bool, error) {
	...
	for _, qdisc := range qdiscs {
		if strings.Contains(qdisc, keyWord) {
			...
			err = c.QdiscDel(uint32(handle), ifIndex)
			if err != nil {
...
func (c *Client) GatewayFiltersDel(gatewayIfIndex int, l3ProtocolVal, l4ProtocolVal int, dstPort uint16, dstIPs []net.IP) error {
	...
	for _, dstIP := range dstIPs {
		...
		cmd := fmt.Sprintf("tc filter del dev %s parent %x:0 prio %d", util.GetNameByIndex(gatewayIfIndex), QdiscHandleIngress, priorityNormal)
		cmd = fmt.Sprintf("%s protocol %s chain %d handle %d flower", cmd, l3ProtocolStr, chain, handle)
		_, err := commandRun(cmd)
		if err != nil {
			return err
		...
		cmd = fmt.Sprintf("tc filter del dev %s parent %x:0 prio %d", util.GetNameByIndex(gatewayIfIndex), QdiscHandleIngress, priorityHigh)
		cmd = fmt.Sprintf("%s protocol %s chain %d handle %d flower", cmd, l3ProtocolStr, chain, handle)
		_, err = commandRun(cmd)
		if err != nil {
			return err
