...
func (b *iptablesRuleBuilder) MatchCIDRSrc(cidr *net.IPNet) IPTablesRuleBuilder {
	if cidr == nil {
		return b
	}
	matchStr := fmt.Sprintf("-s %s", cidr.String())
	b.writeSpec(matchStr)
	return b
}

func (b *iptablesRuleBuilder) MatchCIDRDst(cidr *net.IPNet) IPTablesRuleBuilder {
	if cidr == nil {
		return b
	}
	matchStr := fmt.Sprintf("-d %s", cidr.String())
	b.writeSpec(matchStr)
	return b
}
