	...
	template = &pipelineTemplate{
		stageTables: map[binding.StageID][]tableRequest{
			...
			binding.IngressSecurityStage: {
				tableRequest{IngressClassifierTable, 0x8f},
...
func (c *featureService) getTemplate(protocol ofProtocol) *pipelineTemplate {
	...
	if c.enableProxy {
		...
		if runtime.IsWindowsPlatform() {
			template.stageTables[binding.ClassifierStage] = append(template.stageTables[binding.ClassifierStage], tableRequest{UplinkTable, 0x6f})
		}
		if c.proxyAll {
