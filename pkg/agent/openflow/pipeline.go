...
func (f *featureService) serviceLearnFlow(groupID binding.GroupIDType,
	...
	// If "nodePortAddress" is true, which means that the flow is for NodePort, "ToNodePortAddressRegMark" must be used
	// to match packets. Otherwise, Service IP must be used to match packets.
	if nodePortAddress {
	...
	// If the flow is for external Service IP, which means the Service is accessible externally, the ExternalAddressRegMark
	// should be loaded to determine whether SNAT is required for the connection.
	if externalAddress {
		regMarksToLoad = append(regMarksToLoad, ExternalAddressRegMark)
	}
	...
	for _, ipProtocol := range f.ipProtocols {
		...
		for _, pktDstRegMark := range pktDstRegMarks {
			...
			flows = append(flows, SNATMarkTable.ofTable.BuildFlow(priorityNormal).
				...
				MatchCTStateNew(true).
				MatchCTStateTrk(true).
				MatchRegMark(FromGatewayRegMark, pktDstRegMark, ExternalAddressRegMark).
				Action().CT(true, SNATMarkTable.GetNext(), f.dnatCtZones[ipProtocol], f.ctZoneSrcField).
