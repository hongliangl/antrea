		...
		if enableLogging {
			...
			if l7RuleVlanID != nil {
				return fb.
					...
					LoadToCtMark(L7ANPRedirectCTMark).                               // Mark the packets of the connection should be redirected to an application-aware engine.
					LoadToLabelField(uint64(*l7RuleVlanID), L7ANPRuleVlanIDCTLabel). // Load the VLAN ID allocated for L7 Antrea NetworkPolicy rule to CT mark field L7ANPRuleVlanIDCTMarkField.
					CTDone().
					Cookie(cookieID).
					Done()
			}
