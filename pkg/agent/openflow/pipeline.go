...
var (
	PipelineClassifierTable      = newFeatureTable("PipelineClassifier")
...
func (c *client) defaultFlows(category cookie.Category) []binding.Flow {
	...
	for proto, pipeline := range c.pipelines {
		for _, table := range pipeline.ListAllTables() {
			...
			flows = append(flows, flowBuilder.Cookie(cookieID).Done())
		}
...
func (c *featureService) conntrackFlows(category cookie.Category) []binding.Flow {
	...
	for _, ipProtocol := range c.ipProtocols {
		flows = append(flows,
			...
			ConntrackStateTable.ofTable.BuildFlow(priorityNormal).
				...
				Action().GotoStage(binding.EgressSecurityStage).
				Done(),
			// This generate the flows to load HairpinRegMark for reply packets of SNATTed hairpin connections after passing
			// SNAT ct zone.
	...
	// This generate the flow to match the first packet of Service connections.
...
// New added
// defaultDropFlows set the action of default flow to drop.
...
// Tables: L3ForwardingTable
// New added
// l3FwdFlowToExternal generates the default L3 forward flows to forward packets which are not match by other flows
...
func (c *featureService) l3FwdFlowToExternal(category cookie.Category) binding.Flow {
	...
	// the packets should be rewritten with Antrea gateway's MAC.
...
// Tables: ServiceHairpinMarkTable
// New added
// externalClusterHairpinFlows generate the flow to match packets of hairpin Service connections outside the Cluster.
func (c *featureService) externalClusterHairpinFlows(category cookie.Category) []binding.Flow {
	return []binding.Flow{
