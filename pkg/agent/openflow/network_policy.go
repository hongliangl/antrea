...
func (f *featureNetworkPolicy) initFlows() []binding.Flow {
	...
	if f.nodeType == config.K8sNode {
		...
		if f.enableAntreaPolicy {
			flows = append(flows, f.l7ANPTrafficControlFlows()...)
		}
	}
...
func (f *featureNetworkPolicy) l7ANPTrafficControlFlows() []binding.Flow {
	cookieID := f.cookieAllocator.Request(f.category).Raw()
	vlanMask := uint16(openflow15.OFPVID_PRESENT)
	return []binding.Flow{
		// This generates the flow to output the packets marked with L7ANPRedirectCTMark to an application-aware engine
		// via the target ofPort. Note that, before outputting the packets, VLAN ID stored on field L7ANPRuleVlanIDCTMarkField
		...
		L2ForwardingOutTable.ofTable.BuildFlow(priorityHigh+1).
			...
			MatchCTMark(L7ANPRedirectCTMark).
			Action().PushVLAN(EtherTypeDot1q).
			Action().MoveRange(binding.NxmFieldCtLabel, binding.OxmFieldVLANVID, *L7ANPRuleVlanIDCTLabel.GetRange(), *binding.VLANVIDRange).
