...
func TestBuildPipeline(t *testing.T) {
	...
	for _, tc := range []struct {
		ipStack        ipStack
		features       []feature
		expectedTables map[pipelineProtocol][]*FeatureTable
	}{
		{
			ipStack: dualStack,
			features: []feature{
		...
		templatesMap := make(map[pipelineProtocol][]*featureTemplate)
		...
		for proto, templates := range templatesMap {
			...
			for i := 0; i < len(tables)-1; i++ {
				...
				require.Less(t, tables[i].GetID(), tables[i+1].GetID(), "id of table %s should less than that of table %q", tables[i].GetName(), tables[i+1].GetName())
			}
