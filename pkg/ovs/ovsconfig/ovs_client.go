...
func (br *OVSBridge) updateBridgeConfiguration() error {
	...
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Update(
		update,
		&update.Protocols,
		&update.DatapathType,
...
func (br *OVSBridge) create() error {
	...
	ops2, err := br.ovsdb.Where(conditionOVS(ovs.UUID)).Mutate(mOVS, mutation)
	if err != nil {
...
func (br *OVSBridge) Delete() error {
	...
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Delete()
	if err != nil {
...
func (br *OVSBridge) SetExternalIDs(externalIDs map[string]string) error {
	...
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Update(updateBridge, &updateBridge.ExternalIDs)
	if err != nil {
		klog.ErrorS(err, "Failed to construct update operations for bridge", "bridge", br.name)
		return err
	}
...
func (br *OVSBridge) SetDatapathID(datapathID string) error {
	...
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Mutate(mBridge, mutations...)
	if err != nil {
...
func (br *OVSBridge) WaitForDatapathID(timeout time.Duration) (string, error) {
	...
	mBridge := &Bridge{}
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Wait(ovsdb.WaitConditionNotEqual, &timeoutMs, mBridge, &mBridge.DatapathID)
	if err != nil {
		return "", err
	}
...
func (br *OVSBridge) DeletePort(portUUID string) error {
	...
	ops, err := br.ovsdb.Where(conditionBridge(br.uuid)).Mutate(mBridge, mutation)
	if err != nil {
...
func (br *OVSBridge) SetInterfaceOptions(name string, options map[string]string) error {
	...
	ops, err := br.ovsdb.Where(conditionInterface(name)).Update(updateIntf, &updateIntf.Options)
	if err != nil {
...
func (br *OVSBridge) createPort(name, ifName, ifType string, ofPortRequest int32, vlanID uint16, mac string, externalIDs map[string]string, options map[string]interface{}) (string, error) {
	...
	ops3, err := br.ovsdb.Where(conditionBridge(br.uuid)).Mutate(mBridge, mutation)
	if err != nil {
		klog.ErrorS(err, "Failed to construct mutate operation for bridge", "bridge", br.name)
		return "", err
	}
...
func (br *OVSBridge) GetOFPort(ifName string) (int32, error) {
	...
	mIntf := &Interface{}
	// We use WaitConditionNotEqual to wait for the ofport to be properly assigned by OVS.
	if intf.OFPort != nil {
		ofport := *intf.OFPort
		mIntf.OFPort = &ofport
	}
	ops, err := br.ovsdb.Where(conditionInterface(ifName)).Wait(ovsdb.WaitConditionNotEqual, &timeoutMs, mIntf, &mIntf.OFPort)
	if err != nil {
		return 0, err
	}
...
func (br *OVSBridge) UpdateOVSOtherConfig(configs map[string]string) error {
	...
	ops, err := br.ovsdb.Where(conditionOVS(ovs.UUID)).Mutate(mOVS, mutations...)
	if err != nil {
		return err
	}
...
func (br *OVSBridge) DeleteOVSOtherConfig(keys []string) error {
	...
	// Construct a mutate operation for the Open_vSwitch record.
	ops, err := br.ovsdb.Where(conditionOVS(ovs.UUID)).Mutate(mOVS, mutation)
	if err != nil {
		klog.ErrorS(err, "Failed to construct mutate operation for Open_vSwitch", "bridge", br.name)
		return err
	}
...
func (br *OVSBridge) getHardwareOffload() (bool, error) {
	...
	for configKey, configValue := range otherConfig {
		if configKey == hardwareOffload {
			...
			if err != nil {
				return boolConfigVal, fmt.Errorf("%s", fmt.Sprint("invalid hardwareOffload value: ", boolConfigVal))
			}
...
// We keep this because Interface Options still use map[string]interface{}.
...
func conditionBridge(uuid string) *Bridge       { return &Bridge{UUID: uuid} }
func conditionInterface(name string) *Interface { return &Interface{Name: name} }
func conditionOVS(uuid string) *OpenvSwitch     { return &OpenvSwitch{UUID: uuid} }
