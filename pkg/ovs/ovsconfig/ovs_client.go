...
func (br *OVSBridge) updateBridgeConfiguration() Error {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to update bridge configuration", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) Delete() Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to delete bridge", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetExternalIDs(externalIDs map[string]interface{}) Error {
	...
	// Fetch the Bridge record to mutate its other config.
	bridge, err := br.getBridge(ctx)
	if err != nil {
		return NewTransactionError(err, isTemporaryError(err))
	}
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set external IDs", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetDatapathID(datapathID string) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set datapath ID", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) DeletePorts(portUUIDList []string) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to delete ports", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetInterfaceOptions(name string, options map[string]interface{}) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) createPort(name, ifName, ifType string, ofPortRequest int32, vlanID uint16, mac string, externalIDs, options map[string]interface{}) (string, Error) {
	...
	if options != nil {
		intf.Options = make(map[string]string, len(options))
		for k, v := range options {
			val, ok := v.(string)
			if !ok {
				return "", newInvalidArgumentsError(fmt.Sprintf("invalid interface option value for %s: expected string, got %T", k, v))
			}
			intf.Options[k] = val
		}
	}
	...
	if externalIDs != nil {
		port.ExternalIDs = make(map[string]string, len(externalIDs))
		for k, v := range externalIDs {
			val, ok := v.(string)
			if !ok {
				return "", newInvalidArgumentsError(fmt.Sprintf("invalid external ID value for %s: expected string, got %T", k, v))
			}
	...
	// Submit the batched operations in a single atomic transaction to create the port.
	res, err := br.ovsdb.Transact(ctx, ops...)
	if err != nil {
...
func (br *OVSBridge) GetPortList() ([]OVSPortData, Error) {
	...
	for _, portUUID := range bridge.Ports {
		...
		if !ok {
			klog.Warningf("Failed to get port %s", portUUID)
			continue
		}
		...
		if !ok {
			klog.Warningf("Failed to get interface %s", port.Interfaces[0])
			continue
		}
...
func (br *OVSBridge) AddOVSOtherConfig(configs map[string]interface{}) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to add OVS other config", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) UpdateOVSOtherConfig(configs map[string]interface{}) Error {
	...
	if len(mutateMap) > 0 {
		...
		_, err = br.ovsdb.Transact(ctx, ops...)
		if err != nil {
			klog.ErrorS(err, "Failed to execute transaction to update OVS other config", "bridge", br.name)
			return NewTransactionError(err, isTemporaryError(err))
		}
	}
	return nil
...
func (br *OVSBridge) DeleteOVSOtherConfig(configs map[string]interface{}) Error {
	...
	if len(mutations) > 0 {
		...
		_, err = br.ovsdb.Transact(ctx, ops...)
		if err != nil {
			klog.ErrorS(err, "Failed to execute transaction to delete OVS other config", "bridge", br.name)
			return NewTransactionError(err, isTemporaryError(err))
		}
	}
	return nil
...
func (br *OVSBridge) AddBridgeOtherConfig(configs map[string]interface{}) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to add Bridge other config", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetInterfaceType(name, ifType string) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set interface type", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetPortExternalIDs(portName string, externalIDs map[string]interface{}) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set port external IDs", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
	return nil
}
...
func (br *OVSBridge) SetInterfaceMTU(name string, MTU int) error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set interface MTU", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
...
func (br *OVSBridge) SetInterfaceMAC(name string, mac net.HardwareAddr) Error {
	...
	_, err = br.ovsdb.Transact(ctx, ops...)
	if err != nil {
		klog.ErrorS(err, "Failed to execute transaction to set interface MAC", "bridge", br.name)
		return NewTransactionError(err, isTemporaryError(err))
	}
