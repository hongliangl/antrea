...
func TestValidateAntreaClusterNetworkPolicy(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			operation: admv1.Create,
			expectedWarnings: []string{
				`LogLabels for Node NetworkPolicies are limited to 12 characters, but label "long-long-long-label" for policy rule "rule0" exceeds the limit and will be truncated in kernel logs`,
			},
		},
		{
