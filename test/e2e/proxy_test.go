...
func testProxyLoadBalancerServiceSourceRanges(t *testing.T, isIPv6 bool) {
	...
	// Connect the LoadBalancer ingress IP from test client Pods.
	...
	require.Error(t, probeFromPod(data, notAllowedClient, toolboxContainerName, url), "Service LoadBalancer should not be able to be connected from the Pod")
}
