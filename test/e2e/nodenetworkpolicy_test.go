...
func skipIfNodeNetworkPolicyDisabled(tb testing.TB) {
	skipIfFeatureDisabled(tb, features.NodeNetworkPolicy, true, true)
}
...
// testNodeACNPRejectEgress tests that an ACNP applied to Node drops egress traffic from Node x to Node y when the action
// is REJECT, due to that when using the iptables action REJECT in the OUTPUT chain of the filter table, it defaults to
// the DROP action as an ICMP error message cannot be sent from a host to itself.
